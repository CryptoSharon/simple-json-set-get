...
const jsonfile = {
  readFileSync: path => JSON.parse(Deno.readTextFileSync(path)),
  writeFileSync: (path, data) => Deno.writeTextFileSync(path, JSON.stringify(data, null, 2))
}
import * as path from "https://deno.land/std@0.83.0/path/mod.ts";

import * as Helpers from './settings-helpers.js';
import * as Observer from './settings-observer.js';
 ...
 * @type {string}
 */
// const defaultSettingsDir = path.join(Deno.cwd(), 'data');
const defaultSettingsFilePath = path.join(Deno.cwd(), 'data/config.json');
...
class Settings extends EventEmitter {
  ...
  constructor(path = defaultSettingsFilePath) {
    super();
    ...
    this._defaultSettingsFilePath = path;
    fs.ensureFileSync(this._defaultSettingsFilePath)
   ...
   */
  has(keyPath) {
    assertStrictEquals(typeof keyPath, 'string', 'First parameter must be a string');
  ...
  set(keyPath, value, opts = {}) {
    assertStrictEquals(typeof keyPath, 'string', 'First parameter must be a string. Did you mean to use `setAll()` instead?');
    assertStrictEquals(typeof opts, 'object', 'Second parameter must be an object');
    opts.prettify = true
   ...
   */
  setAll(obj, opts = {}) {
    assertStrictEquals(typeof obj, 'object', 'First parameter must be an object');
    assertStrictEquals(typeof opts, 'object', 'Second parameter must be an object');
  ...
  get(keyPath, defaultValue, opts = {}) {
    assertStrictEquals(typeof keyPath, 'string', 'First parameter must be a string. Did you mean to use `getAll()` instead?');
   ...
   */
  delete(keyPath, opts = {}) {
    assertStrictEquals(typeof keyPath, 'string', 'First parameter must be a string. Did you mean to use `deleteAll()` instead?');
    assertStrictEquals(typeof opts, 'object', 'Second parameter must be an object');
   ...
   */
  deleteAll(opts = {}) {
    assertStrictEquals(typeof opts, 'object', 'First parameter must be an object');
  ...
  watch(keyPath, handler) {
    assertStrictEquals(typeof keyPath, 'string', 'First parameter must be a string');
    assertStrictEquals(typeof handler, 'function', 'Second parameter must be a function');
   ...
   */
  setPath(filePath) {
    assertStrictEquals(typeof filePath, 'string', 'First parameter must be a string');
